...
void CharRegion::ABC_2points(const double A, const double B, const double C, double& x1, double& y1,
  ...
  if (B == 0.) {		// x = xxx
    ...
    set<pair<double, double>> pts_set;
    for (int i = 0; i < 4; ++i) {
      if (xs[i] >= X_LO && xs[i] <= X_UP && ys[i] >= Y_LO && ys[i] <= Y_UP) {
        pts_set.insert(make_pair(xs[i], ys[i]));
      }
    }
    set<pair<double, double>>::iterator it = pts_set.begin();
    x1 = (*it).first;
    y1 = (*it).second;
    ++it;
    x2 = (*it).first;
...
void CharRegion::getNewABLine(const double A, const double B, const double x, const double y,
    double& C) {
  // Ax + By + C = 0;
  C = -(A * x + B * y);
}
