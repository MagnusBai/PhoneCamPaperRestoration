...
template<typename T>
void clac1DHistogram(const vector<T>& data, const int count_hist, const pair<T, T>& range, vector<int>& hist,
    vector<vector<int>>& hist_ids) {
  hist = vector<int>(count_hist, 0);
  hist_ids = vector<vector<int>>(count_hist, vector<int>(0));
  T cell_range = (range.second + static_cast<T>(0.001f) - range.first) / static_cast<T>(count_hist);
...
template<typename T>
void conv1DSignal(const vector<T>& signal, const vector<float>& convFilter, vector<T>& res_signal) {
  ...
  int i_pos = 0;
  for (typename vector<T>::const_iterator it_mid = signal.begin(); it_mid < signal.end(); ++it_mid) {
...
std::vector<SWTPointPair2i> findBoundingBoxes(std::vector<std::vector<SWTPoint2d> > & components,
    std::vector<Chain> & chains, std::vector<SWTPointPair2d> & compBB, Mat& output) {
...
std::vector<SWTPointPair2i> findBoundingBoxes(std::vector<std::vector<SWTPoint2d> > & components, Mat& output) {
  std::vector<SWTPointPair2i> bb;
...
void renderChainsWithBoxes(Mat& SWTImage, std::vector<std::vector<SWTPoint2d> > & components,
    std::vector<Chain> & chains, std::vector<SWTPointPair2d> & compBB, Mat& output) {
...
void renderChains(Mat& SWTImage, std::vector<std::vector<SWTPoint2d> > & components, std::vector<Chain> & chains,
    Mat& output) {
  // keep track of included components
  std::vector<bool> included;
  included.reserve(components.size());
...
//	imwrite("SWT.png", saveSWT);
    ...
    for (auto& pt : char_vec) {
      ...
      for (int h_ = h0_ - plot_scope; h_ < h0_ + plot_scope; ++h_) {
        for (int w_ = w0_ - plot_scope; w_ < w0_ + plot_scope; ++w_) {
          if (h_ > 0 && w_ > 0 && h_ < char_plot.rows && w_ < char_plot.cols) {
  ...
  int count_ids = regionGrowth_pro(&ipl_char_plot, pres_mat, ipl_char_plot.width, ipl_char_plot.height, seedx, seedy,
      50);
  ...
  for (int i = 0; i < count_char_regions; ++i) {
    charRegionArray.push_back(
        CharRegion(count_region_pixs[i], count_region_contours[i], count_char_edge[i], x_max[i], x_min[i], y_max[i],
            y_min[i]));
  }
...
//		// draw edge
    for (int j = 0; j < charRegionArray[i].count_edges; ++j) {
      ...
      lines_im.at<cv::Vec3b>(y, x)[0] = CharRegion::solarized_palette["base03"][0];
      lines_im.at<cv::Vec3b>(y, x)[1] = CharRegion::solarized_palette["base03"][1];
      lines_im.at<cv::Vec3b>(y, x)[2] = CharRegion::solarized_palette["base03"][2];
    }
    // draw line
    ...
    for (int j = 0; j < charRegionArray[i].const_nlines; ++j) {
      ...
      cv::line(lines_im,
          ...
          cv::Point(charRegionArray[i].line_paramsPts_global[j * 4 + 2],
              charRegionArray[i].line_paramsPts_global[j * 4 + 3]), CharRegion::solarized_palette["yellow"], 1);
    }
  ...
  for (int i = 0; i < count_char_regions; ++i) {
    ...
    angles_scores_set.insert(angles_scores_set.end(), newbies.begin(), newbies.end());
  }
  cout << endl << endl << endl;
...
//	vector<int>& first_class_id = rslope_id_hierachy[0];
  ...
  clac1DHistogram<float>(angles_vector, count_bins, angle_range, hist, hist_hierarchy);
...
//										);
  ...
  CharRegion::twoPoints_ABC(x1_aux_line, y1_aux_line, x2_aux_line, y2_aux_line, A_aux_line, B_aux_line, C_aux_line);
  ...
  for (auto& id : cand_line_ids) {
    ...
    line(lines_im, Point(cr.bb_intersection_pts[lineid * 4 + 0], cr.bb_intersection_pts[lineid * 4 + 1]),
...
//		double c = base_line_pre_det.coefs[2];
  ...
  double sum_dist = 0.;
  std::for_each(distance_array.begin(), distance_array.end(), [&] (double n) {
    sum_dist += n;
  });
  ...
  line(lines_im, Point(x1_base_line, y1_base_line), Point(x2_base_line, y2_base_line),
      CharRegion::solarized_palette["magenta"], 5);

  double A_base_line, B_base_line, C_base_line;
  CharRegion::twoPoints_ABC(x1_base_line, y1_base_line, x2_base_line, y2_base_line, A_base_line, B_base_line,
      C_base_line);
  ...
  CharRegion::getVerticalLine(A_base_line, B_base_line, C_base_line, mid_base_line_x, mid_base_line_y, A_base_line_vert,
  ...
  CharRegion::ABC_2points(A_base_line_vert, B_base_line_vert, C_base_line_vert, x1_base_line_vert, y1_base_line_vert,
      x2_base_line_vert, y2_base_line_vert);

  cout << A_base_line << "x + " << B_base_line << "y + " << C_base_line << " = 0" << endl;
  cout << x1_base_line << " , ";
  cout << y1_base_line << " , ";
  cout << x2_base_line << " , ";
  cout << y2_base_line << endl;
  cout << A_base_line_vert << "x + " << B_base_line_vert << "y + " << C_base_line_vert << " = 0" << endl;
  cout << x1_base_line_vert << " , ";
  cout << y1_base_line_vert << " , ";
  cout << x2_base_line_vert << " , ";
  cout << y2_base_line_vert << endl;

  line(lines_im, Point(x1_base_line_vert, y1_base_line_vert), Point(x2_base_line_vert, y2_base_line_vert),
      CharRegion::solarized_palette["orange"], 5);
...
void strokeWidthTransform(const Mat& edgeImage, Mat& gradientX, Mat& gradientY, bool dark_on_light, Mat& SWTImage,
    std::vector<Ray> & rays) {
  // First pass
  float prec = .05;
          ...
          if ((int) (floor(curX)) != curPixX || (int) (floor(curY)) != curPixY) {
            ...
            if (edgeImage.at<uchar>(curPixY, curPixX) > 0) {
              ...
              if (acos(G_x * -G_xt + G_y * -G_yt) < PI / 2.0) {
                float length = sqrt(
                    ((float) r.q.x - (float) r.p.x) * ((float) r.q.x - (float) r.p.x)
...
std::vector<std::vector<SWTPoint2d> > findLegallyConnectedComponentsRAY(Mat& SWTImage, std::vector<Ray> & rays) {
  boost::unordered_map<int, int> map;
  boost::unordered_map<int, SWTPoint2d> revmap;
...
void componentStats(Mat& SWTImage, const std::vector<SWTPoint2d> & component, float & mean, float & variance,
    float & median, int & minx, int & miny, int & maxx, int & maxy) {
...
void filterComponents(Mat& SWTImage, std::vector<std::vector<SWTPoint2d> > & components,
    std::vector<std::vector<SWTPoint2d> > & validComponents, std::vector<Point2dFloat> & compCenters,
    std::vector<float> & compMedians, std::vector<SWTPoint2d> & compDimensions, std::vector<SWTPointPair2d> & compBB) {
...
std::vector<Chain> makeChains(const Mat& colorImage, std::vector<std::vector<SWTPoint2d> > & components,
    std::vector<Point2dFloat> & compCenters, std::vector<float> & compMedians, std::vector<SWTPoint2d> & compDimensions,
    std::vector<SWTPointPair2d> & compBB) {
  ...
  for (unsigned int i = 0; i < components.size(); i++) {
    for (unsigned int j = i + 1; j < components.size(); j++) {
      ...
      if ((compMedians[i] / compMedians[j] <= 2.0 || compMedians[j] / compMedians[i] <= 2.0)
        ...
        if (dist
            < 9
                * (float) (std::max(std::min(compDimensions[i].x, compDimensions[i].y),
                    std::min(compDimensions[j].x, compDimensions[j].y)))
                * (float) (std::max(std::min(compDimensions[i].x, compDimensions[i].y),
                    std::min(compDimensions[j].x, compDimensions[j].y))) && colorDist < 1600) {
          Chain c;
          c.p = i;
          c.q = j;
        ...
        if (i != j) {
          if (!chains[i].merged && !chains[j].merged && sharesOneEnd(chains[i], chains[j])) {
            if (chains[i].p == chains[j].p) {
              if (acos(chains[i].direction.x * -chains[j].direction.x + chains[i].direction.y * -chains[j].direction.y)
                  < strictness) {
                ...
                for (std::vector<int>::iterator it = chains[j].components.begin(); it != chains[j].components.end();
                    it++) {
